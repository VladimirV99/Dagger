...
namespace jss {
    namespace detail {
        ...
        struct HasSmartPointerOps {
            ...
            template <typename Archetype> struct TrueTest { FalseTest dummy[2]; };

            template <typename Archetype> static FalseTest TestOpStar(...);
            template <typename Archetype>
            static TrueTest<decltype(*std::declval<Archetype const&>())>
                TestOpStar(Archetype*);

            template <typename Archetype> static FalseTest TestOpArrow(...);
            template <typename Archetype>
            static TrueTest<decltype(std::declval<Archetype const&>().operator->())>
                TestOpArrow(Archetype*);

            template <typename Archetype> static FalseTest TestGet(...);
            template <typename Archetype>
            static TrueTest<decltype(std::declval<Archetype const&>().get())>
                TestGet(Archetype*);

            static constexpr bool s_Value =
                !std::is_same<decltype(TestGet<Ptr>(0)), FalseTest>::value &&
                !std::is_same<
                decltype(TestOpArrow<Ptr>(0)), FalseTest>::value &&
                !std::is_same<
                decltype(TestOpStar<Ptr>(0)), FalseTest>::value;
        };
        ...
        struct IsSmartPointer
            : std::integral_constant<
            bool, SmartPointerOpsConsistent<Ptr>::value> {};
    ...
    /// convertible to T*
    template <typename Archetype> class ObjectPtr {
    public:
        ...
        template <
            ...
            detail::IsConvertibleSmartPointer<Ptr, Archetype>::value>>
        ...
        friend constexpr bool
            operator==(ObjectPtr const& lhs_, ObjectPtr const& rhs_) noexcept {
...
namespace std {
    ...
    template <typename Archetype> struct hash<jss::ObjectPtr<Archetype>> {
        constexpr size_t operator()(jss::ObjectPtr<Archetype> const& p_) const
            noexcept {

...
Bool PingPongAISystem::ShouldConsiderBall(const Transform& ballTransform_, const PingPongBall& ball_, const Transform& playerTransform_, const AI& playerAI_)
{
    if (playerAI_.side == EPlayerSide::LEFT)
    {
        return !ball_.processed &&
            ball_.speed.x < 0 &&
            ballTransform_.position.x < 0 &&

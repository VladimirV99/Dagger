...
void ping_pong::SetupWorld()
{
	...
	// Collisions
	{
		// Up
		{
			...
			transform.position.y =
				(0.5f + (height - 1) + (height - 1) * space - static_cast<float>(height * (1 + space)) / 2.f) *
				tileSize;
			transform.position.z = zPos;
		}
		...
		// Right
		{
			...
			transform.position.x =
				(0.5f + (width - 1) + (width - 1) * space - static_cast<float>(width * (1 + space)) / 2.f) * tileSize;
			transform.position.y = 0;
			transform.position.z = zPos;
	...
	// 2nd player
	{
		...
		auto& transform = reg.emplace<Transform>(entity);
		transform.position.x =
			(0.5f + (width - 3) + (width - 1) * space - static_cast<float>(width * (1 + space)) / 2.f) * tileSize;
		transform.position.y = 0;
		transform.position.z = zPos;

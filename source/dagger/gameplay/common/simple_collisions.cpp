...
bool SimpleCollision::IsCollided(const Vector3& pos_, const SimpleCollision& other_, const Vector3& posOther_) const
{
	...
	return p.x < p2.x + other_.size.x && p.x + size.x > p2.x && p.y < p2.y + other_.size.y && p.y + size.y > p2.y;
}

Vector2
SimpleCollision::GetCollisionSides(const Vector3& pos_, const SimpleCollision& other_, const Vector3& posOther_) const
{
	Vector2 res(0, 0);
...
Vector3
SimpleCollision::GetCollisionCenter(const Vector3& pos_, const SimpleCollision& other_, const Vector3& posOther_) const
{
	Vector3 res;
